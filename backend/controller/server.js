...
app.post('/upload-image', upload.single('image'), (req, res) => {
    ...
    exec(`"${condaPath}" run -n dl python blip_caption.py "${imagePath}"`, async (error, stdout, stderr) => {
        if (error) {
            console.error(`exec error: ${error}`);
            return res.status(500).send('Server Error');
        }
        ...
        exec(`"${condaPath}" run -n dl python clip_search.py "${description}"`, async (clipError, clipStdout, clipStderr) => {
            if (clipError) {
